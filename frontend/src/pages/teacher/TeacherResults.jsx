...
export default function TeacherResults() {
  ...
  const handleDelete = async id => { if(!confirm("Delete?")) return; await fetch(`${API}/api/results/${id}`,{method:"DELETE"}); fetch$(); };

  return (
    <div className="admin-page-content">
      <div className="admin-page-header split">
        <div><h1>Results</h1><p>{results.length} result{results.length!==1?"s":""}</p></div>
        <button className="admin-btn-dark" onClick={()=>setShowForm(!showForm)}><i className="ri-add-line"/> Add Result</button>
      </div>
      ...
      <div className="admin-table-container">
        <table className="admin-table">
          ...
          <tbody>
            {results.length===0?(<tr><td colSpan="9" className="admin-table-empty">No results.</td></tr>)
            :results.map(r=>(
              <tr key={r._id}>
                ...
                <td><button onClick={()=>handleDelete(r._id)} style={{background:"#fee2e2",color:"#dc2626",border:"none",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:"0.8rem",fontWeight:600}}><i className="ri-delete-bin-line"/></button></td>
              </tr>
            ))}
          </tbody>
        </table>
